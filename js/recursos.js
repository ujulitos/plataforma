        ...
        firebase.database().ref(laUrlBase + 'Recursos').once('value').then(function(snapshot) {
            if (snapshot.val() != null) {
                ...
                snapshot.forEach(function(childSnapshot) {
                    ...
                    console.log('recursoNombre ' + contadorRecursos + ': ', this['recursoNombre' + contadorRecursos]);
                    ...
                    // console.log('Descripcion del Curso: ', this['recursoDesc' + contadorRecursos]);
...
function pintaCursos() {
    ...
    var anchoColumnas = 12;
    // if (cuantosRecursosVisibles >= 3) {
    // if (cuantosRecursos >= 7) {
    //     anchoColumnas = 6;
    // }
    ...
    for (a = 1; a <= cuantasCategorias; a++) {
        ...
        contenidoSecCursos += '<div class="col-md-12 ml-auto mr-auto" style="width: 95%;">';
        ...
        for (b = 1; b <= cuantosRecursos; b++) {
            ...
            if (that['cursoCategoria' + b] == arrayCategorias[(a - 1)]) {
                ...
                if (that['cursoNivel' + b] <= elUsuarioNivel) {
                    ...
                    contenidoSecCursos += '<div id="tarjeta' + b + '" class="card card_recursos">';
                    contenidoSecCursos += '<div class="card-text">';
                    contenidoSecCursos += '<div class="portada" style="background-image: url(' + this['cursoPortada' + b] + '?' + noCache + ');"></div>';
                    contenidoSecCursos += '<div class="title-total">';
                    contenidoSecCursos += '<div id="categoria_curso' + b + '" class="icono_categoria">';
                    if (this['cursoCategoria' + b] === 'Video') {
                        contenidoSecCursos += '<i class="material-icons-outlined">videocam</i>';
                    }
                    if (this['cursoCategoria' + b] === 'eLearning') {
                        contenidoSecCursos += '<i class="material-icons">laptop_chromebook</i>';
                    }
                    contenidoSecCursos += '</div>';
                    contenidoSecCursos += '<h4>' + this['recursoNombre' + b] + '</h4>';
                    if (this['recursoDesc' + b] != null && this['recursoDesc' + b] != ' ') {
                        contenidoSecCursos += '<br><div class="desc"><b>Descripción: </b>' + this['recursoDesc' + b] + '</div>';
                    }
                    if (this['cursoDurac' + b] != null && this['cursoDurac' + b] != ' ') {
                        contenidoSecCursos += '<div class="durac"><b>Duración: </b>' + this['cursoDurac' + b] + '</div>';
                    }
                    if (this['cursoObj' + b] != null) {
                        contenidoSecCursos += '<p class="card-text" ><b>Objetivos: </b>' + this['cursoObj' + b] + '</p>';
                    }
                    if (this['cursoTemario' + b] != null) {
                        contenidoSecCursos += '<p class="card-text" ><b>Temario: </b>' + this['cursoTemario' + b] + '</p>';
                    }
                    contenidoSecCursos += '<div class="actions">';
                    contenidoSecCursos += '<div id="estatus_curso_scorm' + b + '" class="icono_status"><i class="material-icons icon_estatus_scorm">radio_button_unchecked</i></div>';
                    if (this['cursoVisible' + b]) {
                        contenidoSecCursos += '<div id="botonIniciarCurso' + b + '" class="btn btn-verde btn-round botonIniciarCurso">Lanzar</div>';
                    } else {
                        contenidoSecCursos += '<div id="botonIniciarCurso' + b + '" class="btn btn-verde btn-round botonIniciarCurso btn-disabled" disabled>Lanzar</div>';
                    }
                    contenidoSecCursos += '</div>';
                    contenidoSecCursos += '</div>';
                    contenidoSecCursos += '</div>';
